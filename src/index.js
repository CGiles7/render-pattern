...
function calculateTotal() {
  const total = books.reduce((acc, book) => acc + book.price * book.quantity, 0);
...
function render() {
  ...
  if (books.length > 0) {
    books.forEach((book) => {
      cartItemsHTML += renderBook(book);
    });
  } else {
    cartItemsHTML = '<p>Nothing in cart</p>';
  }

  cartItemsSection.innerHTML = cartItemsHTML;

  // Calculate and render the total in dollars (no cents)
  const total = calculateTotal();
  const totalInDollars = parseInt(total);
  totalSection.textContent = `$${totalInDollars}`;
}
